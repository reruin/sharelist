...
class wrapReadableStream extends Readable {
  constructor(target , options) {
    ...
    this.target.on('response' , (response)=>{
  ...
  _read() {
    ...
    if( n > 0 && n <= this.length ){
      ...
      if(r == false){
        if(this.isTargetPaused() == false) this.target.pause()
      }
      else{
...
module.exports = function(target , options){
  return new wrapReadableStream(target , options)
}